...
def read(fp, mem="RAM", binfactor=1, ft=None, **kwargs):
    ...
    assert(ft in [None,'dm','empad','mrc_relativity','gatan_K2_bin','kitware_counted','dmstems']), "Error: ft argument not recognized"
    ...
    if ft == "py4DSTEM":
        ...
        from .nonnative.read_dmstems import read_dmstems
        data, md = read_dmstems(fp, mem, binfactor, **kwargs)
    else:
